...
| **3. Transport & protocol** | `MotionUart`, `CommInterface`, `StreamInterface`, `UartPacket`, `CommandError` | 18 | sw + hw | Adequate | `test_errors.py` (CRC, CommandError), `test_comm_paths.py` (path isolation, hw), `test_comm_transport_down.py` (cancellation, sw). Back-pressure: `test_usb_drain_stall.py` (HIL sweep — how long a host stall the firmware tolerates, issue #116) + `test_usb_drain_stall_unit.py` (its classifier, sw). Hole: UART CRC-corruption + timeout HIL. |
