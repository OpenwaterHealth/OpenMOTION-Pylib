...
| **3. Transport & protocol** | `MotionUart`, `CommInterface`, `StreamInterface`, `UartPacket`, `CommandError` | 18 (+4) | sw + hw | Adequate | `test_errors.py` (CRC, CommandError), `test_comm_paths.py` (path isolation, hw), `test_comm_transport_down.py` (cancellation, sw), `test_stream_image_routing.py` (drip-scan TYPE_IMAGE framing/routing — histogram hot path unchanged when no image queue is attached, sw). Hole: UART CRC-corruption + timeout HIL. |
...
| **10. Drip-scan image capture** | `ImageCapture` (line parser, RAW10 unpacker, `FrameAssembler`, `FpgaRegs`, group-hold retiming, capture orchestrator) | 29 | sw | **Strong (sw); HIL-deferred** | `test_image_capture.py`: per-line CRC (`util_crc16`, check value 0x29B1) + RAW10 bit layout via independently-computed golden vectors (line CRC 0xA25E), `FrameAssembler` single-exposure `frame_cnt` enforcement + overrun/wedge tracking, FPGA map-v2 sweep/overrun/wedge registers, atomic group-hold retiming sequence, and the pure `next_sweep_action` retry policy. The `capture_full_frames` orchestrator body is HIL-verified (spec §5: bit-exact test-pattern sweep, laser-synced single-exposure frame, histogram-suite regression) — companion to camera-fpga#8 / sensor-fw. |
