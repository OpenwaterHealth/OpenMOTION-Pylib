...
def _make_fake_scan(left, right):
    ...
    Spawns a thread that:
      1. Emits synthetic uncorrected Samples via on_uncorrected_fn —
         matching the live-capture flow used by CalibrationWorkflow.
    ...
    def _emit(on_uncorrected_fn):
        if on_uncorrected_fn is None:
            return
            ...
            for cam_id in range(8):
                ...
                for fid in range(50, 100):
                    on_uncorrected_fn(Sample(
                        ...
                        bfi=5.0, bvi=5.0,
                        is_corrected=False, is_dark=False,
                    ))

    def _fake(req, *, on_complete_fn=None, on_log_fn=None,
              on_uncorrected_fn=None, **kw):
        def _run():
            time.sleep(0.05)
            _emit(on_uncorrected_fn)
            res = ScanResult(
