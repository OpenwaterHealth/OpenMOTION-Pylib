...
class I2C_DATA_Packet:
    ...
    def pkt_len(self):
        ...
        return (
            struct.calcsize("<BHBBB") + self.data_len + 2
        )  # Packet header + data length + CRC size

    def calculate_crc(self):
        crc16 = crcmod.predefined.Crc("crc-ccitt-false")
        buffer = struct.pack(
            "<BHBBB", self.pkt_len, self.id, self.cmd, self.reserved, self.data_len
        )
        buffer += self.pData
        crc16.update(buffer)
    ...
    def from_buffer(self, buffer):
        ...
        pktLen = 0
        pktLen, self.id, self.cmd, self.reserved, self.data_len = struct.unpack(
            "<BHBBB", buffer[:6]
        )
