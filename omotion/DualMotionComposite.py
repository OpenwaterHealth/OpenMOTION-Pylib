...
logger = logging.getLogger(
    f"{_log_root}.DualMotionComposite" if _log_root else "DualMotionComposite"
)
...
class DualMotionComposite(SignalWrapper):
    ...
    def connect(self, target=None):
        ...
        if not devices:
            logger.debug(
                f"No sensor devices found (VID=0x{self.vid:X}, PID=0x{self.pid:X})"
            )
            return
        ...
        for dev in devices:
            try:
                ...
                if not port_numbers:
                    logger.warning(
                        f"Device at bus {dev.bus} addr {dev.address} has no port_numbers; skipping"
                    )
                    continue
                ...
                if port_numbers[-1] == 2:
                    if not self.left and target == "left":
                        logger.debug(
                            f"Connecting LEFT sensor (bus {dev.bus}, ports {port_numbers})"
                        )
                        self._left_dev = dev
                        self.left = MotionComposite(
                            dev, desc="LEFT", async_mode=self.async_mode
                        )
                ...
                # Right sensor (port ends with 3)
                    ...
                    if not self.right and target == "right":
                        logger.debug(
                            f"Connecting RIGHT sensor (bus {dev.bus}, ports {port_numbers})"
                        )
                        self._right_dev = dev
                        self.right = MotionComposite(
                            dev, desc="RIGHT", async_mode=self.async_mode
                        )
    ...
    def check_usb_status(self):
        ...
        try:
            ...
            if (
                not any(getattr(dev, "port_numbers", [])[-1] == 2 for dev in devices)
                and self.left
            ):
            ...
            if (
                not any(getattr(dev, "port_numbers", [])[-1] == 3 for dev in devices)
                and self.right
            ):
                logger.info("Disconnecting right sensor")
                self.disconnect(target="right")
        except Exception:
            logger.error("Error disconnecting from sensor device")
